...
public class RazorpayDelegate implements ActivityResultListener, ExternalWalletListener, PaymentResultWithDataListener {
    ...
    public RazorpayDelegate(Activity activity) {
        ...
        if (activity instanceof ComponentActivity) {
            checkoutLauncher = ((ComponentActivity) activity).registerForActivityResult(
                new ActivityResultContracts.StartActivityForResult(),
                result -> handleCheckoutResult(Checkout.RZP_REQUEST_CODE, result.getResultCode(), result.getData())
            );
        } else {
    ...
    private void launchCheckoutLegacy(Intent intent) {
        try {
            Method startActivityForResult = Activity.class.getMethod(
                "startActivityForResult",
                Intent.class,
                Integer.TYPE
            );
            startActivityForResult.invoke(activity, intent, Checkout.RZP_REQUEST_CODE);
        } catch (Exception exception) {
            Map<String, Object> reply = new HashMap<>();
            reply.put("type", CODE_PAYMENT_ERROR);

            Map<String, Object> data = new HashMap<>();
            data.put("code", UNKNOWN_ERROR);
            data.put("message", "Unable to launch Razorpay checkout.");
            data.put("responseBody", exception.getMessage());
    ...
    public boolean onActivityResult(int requestCode, int resultCode, Intent data) {
        handleCheckoutResult(requestCode, resultCode, data);
        return true;
    }

    private void handleCheckoutResult(int requestCode, int resultCode, Intent data) {
        try {
            ...
            try {
                Method legacyResultHandler = Checkout.class.getMethod(
                ...
                legacyResultHandler.invoke(null, activity, requestCode, resultCode, data, this, this);
            } catch (Exception legacyException) {
                throw new RuntimeException("Unable to process Razorpay checkout result.", legacyException);
            }
        }
    }
