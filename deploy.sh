...
if [ "$BUILD" = true ]; then
  ...
  ssh "myvps" "cd $VM_DIR && docker-compose up -d --build api"
else
  echo "==> Restarting api container (no rebuild)..."
  ssh "myvps" "cd $VM_DIR && docker-compose restart api"
fi
...
if [ "$WEB" = true ]; then
  ...
  ssh "myvps" "cd $VM_DIR && docker-compose exec nginx nginx -s reload"
fi
